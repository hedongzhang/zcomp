...
#define CHUNK 512 * 16
#define COMP_LEVEL 1    //ZSTD_CLEVEL_DEFAULT
#define USE_DICT 1
...
int main(int argc, char *argv[])
{
        ...
        if(strlen(file_input) > 5) {
                if(strcmp(file_input + strlen(file_input) - 5, ".zstd") == 0) {
                        compress = 0;
                        strncat(file_output, file_input, strlen(file_input) - 5);
                } else {
                        compress = 1;
                        strncat(file_output, file_input, strlen(file_input));
                        strncat(file_output, ".zstd", sizeof(".zstd"));
                }
        } else {
                compress = 1;
                strncat(file_output, file_input, strlen(file_input));
                strncat(file_output, ".zstd", strlen(".zstd"));
        }
        ...
        // if(zstd_init(ZSTD_minCLevel(), USE_DICT)) {
        if(zstd_init(COMP_LEVEL, USE_DICT)) {
                ret = -1;
                goto final1;
        }
        ...
        while (size) {
                if(compress) {
                        ...
                        ret = zstd_comp(buff_out, CHUNK * 2, buff_in, size);
                        if(ret < 0) {
                                printf("zstd_comp failed ret:%d\n", ret);
                                ret = -1;
                                goto final2;
                        }
                        ...
                        *((uint32_t*)out) = ret;
                        ...
                        ret = zstd_decomp(buff_out, CHUNK * 2, buff_in, size);
                        if(ret < 0) {
                                ret = -1;
                                printf("zstd_decomp failed:%d\n", errno);
                                goto final2;
                        }
